...
<div class={`min-w-[260px] rounded-xl border p-3 text-card-foreground shadow-sm ${stateClass} ${selected ? 'ring-2 ring-ring' : ''}`}>
  ...
  <div class="flex items-start justify-between gap-3">
    ...
    <span class="rounded-md bg-background/70 px-1.5 py-0.5 text-xs text-muted-foreground">P{data.priority}</span>
  </div>
  <p class="mt-2 line-clamp-2 text-xs text-muted-foreground" title={data.description}>{data.description || 'No description'}</p>
