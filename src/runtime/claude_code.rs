...
fn claude_config_path() -> Result<PathBuf> {
    ...
    if let Some(path) = std::env::var_os("PONTIA_CLAUDE_CONFIG_PATH") {
...
mod tests {
    ...
    #[test]
    fn config_path_uses_pontia_override_when_set() {
        // PONTIA_CLAUDE_CONFIG_PATH is intentionally test-only; this verifies
        // tests can redirect config writes without touching ~/.claude.json.
        let _lock = env_lock();
        let dir = tempdir().expect("tempdir");
        let config_path = dir.path().join("custom-claude.json");
        unsafe {
            std::env::set_var("PONTIA_CLAUDE_CONFIG_PATH", &config_path);
        }

        assert_eq!(claude_config_path().expect("config path"), config_path);

        unsafe {
            std::env::remove_var("PONTIA_CLAUDE_CONFIG_PATH");
        }
    }
