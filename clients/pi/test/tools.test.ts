...
import { buildPontiaTools, internalAgentToolUrl } from "../src/tools.js";
...
function install(env: Record<string, string | undefined> = {}, overrides: Partial<Parameters<typeof createPontiaPiExtension>[1]> = {}) {
...
describe("pontia pi agent tools", () => {
  ...
  test("does not register pontia tools when agent kind is missing", () => {
  ...
  test("does not register pontia tools when agent kind is unsupported", () => {
    const { pi, tools } = install({ PONTIA_AGENT_KIND: "unknown" });
  ...
  test("planner agent kind registers planning tools only", () => {
    const { pi, tools } = install({ PONTIA_AGENT_KIND: "planner" });
  ...
  test("executor agent kind registers execution tools only", () => {
    const { pi, tools } = install({ PONTIA_AGENT_KIND: "executor" });
  ...
  test("tool handler posts current authorized context and input to Internal Agent Tool API", async () => {
    ...
    const { tools } = install({ PONTIA_AGENT_KIND: "planner" }, { fetch: fetchImpl as any });
  ...
  test("all tool handlers return agent-visible plain text instead of JSON", async () => {
    const responses: Record<string, unknown> = {
      getContext: { ok: true, tool: "getContext", result: { text: "pontia context: execution\n\nCurrent WorkItem:\n- Title: Do it" } },
  ...
  test("getContext returns agent-visible plain text instead of JSON", async () => {
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({ ok: true, tool: "getContext", result: { text: "pontia context: execution\n\nCurrent WorkItem:\n- Title: Do it" } }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }),
    );
    const { tools } = install({ PONTIA_AGENT_KIND: "executor" }, { fetch: fetchImpl as any });

    const result = await tools.find((tool) => tool.name === "getContext")!.execute("call_context", {});

    expect(result.content[0].text).toBe("pontia context: execution\n\nCurrent WorkItem:\n- Title: Do it");
    expect(result.content[0].text).not.toContain("{\n");
    expect(result.details).toEqual({ ok: true, tool: "getContext", result: { text: "pontia context: execution\n\nCurrent WorkItem:\n- Title: Do it" } });
  });
  ...
  test("backend error is returned as a tool error without leaking environment", async () => {
    ...
    const { tools } = install(
      { PONTIA_AGENT_KIND: "executor", PONTIA_EXTERNAL_API_TOKEN: "secret-token" },
